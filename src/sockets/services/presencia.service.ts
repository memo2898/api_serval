...
export class PresenciaService {
  ...
  registrar(socket_id: string, usuario_id: number, nombre: string, sucursal_id: number, mesa_id: number) {
    ...
    if (existing) {
      ...
      this.presencias.set(socket_id, { usuario_id, nombre, sucursal_id, mesas: [mesa_id] });
    }
  }
  ...
  getSalida(socket_id: string): PresenciaData {
    return this.presencias.get(socket_id) ?? { usuario_id: 0, nombre: '', sucursal_id: 0, mesas: [] };
  }
  ...
  getAll(sucursal_id: number) {
    const result: Record<number, { usuario_id: number; nombre: string }[]> = {};
    for (const [, data] of this.presencias) {
      ...
      for (const mesa_id of data.mesas) {
        ...
        result[mesa_id].push({ usuario_id: data.usuario_id, nombre: data.nombre });
      }
    }
