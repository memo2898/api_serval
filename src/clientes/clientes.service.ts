import {
  Injectable,
  NotFoundException,
  HttpException,
  HttpStatus,
} from '@nestjs/common';
import { InjectRepository } from '@nestjs/typeorm';
import { Repository, SelectQueryBuilder } from 'typeorm';
import { CreateClienteDto } from './dto/create-cliente.dto';
import { UpdateClienteDto } from './dto/update-cliente.dto';
import { ClienteFiltersDto } from './dto/pagination.dto';
import { Cliente } from './entities/cliente.entity';
import { PaginationResponse } from './interfaces/pagination-response.interface';

@Injectable()
export class ClientesService {
  constructor(
    @InjectRepository(Cliente)
    private clientesRepository: Repository<Cliente>,
  ) {}

  private getErrorMessage(error: unknown): string {
    if (error instanceof Error) return error.message;
    if (typeof error === 'string') return error;
    return 'Error desconocido';
  }

  async create(dto: CreateClienteDto) {
    try {
      const record = this.clientesRepository.create(dto);
      return await this.clientesRepository.save(record);
    } catch (error: unknown) {
      throw new HttpException(
        `Error al crear el registro: ${this.getErrorMessage(error)}`,
        HttpStatus.BAD_REQUEST,
      );
    }
  }

  /**
   * Sin query params  → retorna un array plano: [{}, {}, {}]
   * Con cualquier param → retorna { data: [...], meta: {...} }
   */
  async findAll(
    filters: ClienteFiltersDto,
  ): Promise<Cliente[] | PaginationResponse<Cliente>> {
    try {
      const hasParams = Object.values(filters).some(
        (v) => v !== undefined && v !== null && v !== '',
      );

      if (!hasParams) {
        return await this.clientesRepository.find();
      }

      const { page, limit, sort, ...filterParams } = filters;

      const qb = this.clientesRepository.createQueryBuilder('cliente');
      this.applyFilters(qb, filterParams);
      this.applySorting(qb, sort);

      if (page !== undefined && limit !== undefined) {
        const p = Number(page);
        const l = Number(limit);
        const [data, total] = await qb
          .skip((p - 1) * l)
          .take(l)
          .getManyAndCount();
        const totalPages = Math.ceil(total / l);
        return {
          data,
          meta: {
            total,
            page: p,
            limit: l,
            totalPages,
            hasNext: p < totalPages,
            hasPrev: p > 1,
          },
        };
      }

      const data = await qb.getMany();
      return {
        data,
        meta: {
          total: data.length,
          page: 1,
          limit: data.length,
          totalPages: 1,
          hasNext: false,
          hasPrev: false,
        },
      };
    } catch (error: unknown) {
      throw new HttpException(
        `Error al obtener los registros: ${this.getErrorMessage(error)}`,
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }

  async findOne(id: number) {
    try {
      const record = await this.clientesRepository.findOne({ where: { id } });
      if (!record) {
        throw new NotFoundException(`Cliente con ID ${id} no encontrado`);
      }
      return record;
    } catch (error: unknown) {
      if (error instanceof NotFoundException) throw error;
      throw new HttpException(
        `Error al obtener el registro: ${this.getErrorMessage(error)}`,
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }

  async update(id: number, dto: UpdateClienteDto) {
    try {
      const existing = await this.clientesRepository.findOne({ where: { id } });
      if (!existing) {
        throw new NotFoundException(`Cliente con ID ${id} no encontrado`);
      }
      await this.clientesRepository.update(id, dto);
      return await this.clientesRepository.findOne({ where: { id } });
    } catch (error: unknown) {
      if (error instanceof NotFoundException) throw error;
      throw new HttpException(
        `Error al actualizar el registro: ${this.getErrorMessage(error)}`,
        HttpStatus.BAD_REQUEST,
      );
    }
  }

  async remove(id: number) {
    try {
      const existing = await this.clientesRepository.findOne({ where: { id } });
      if (!existing) {
        throw new NotFoundException(`Cliente con ID ${id} no encontrado`);
      }
      await this.clientesRepository.delete(id);
      return { message: `Cliente con ID ${id} eliminado correctamente` };
    } catch (error: unknown) {
      if (error instanceof NotFoundException) throw error;
      throw new HttpException(
        `Error al eliminar el registro: ${this.getErrorMessage(error)}`,
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }

  private applyFilters(
    qb: SelectQueryBuilder<Cliente>,
    filters: Partial<ClienteFiltersDto>,
  ) {
    if (filters.id !== undefined) {
      qb.andWhere('cliente.id = :id', { id: filters.id });
    }
    if (filters.empresa_id !== undefined) {
      qb.andWhere('cliente.empresa_id = :empresa_id', {
        empresa_id: filters.empresa_id,
      });
    }
    if (filters.nombre) {
      qb.andWhere('cliente.nombre LIKE :nombre', {
        nombre: `%${filters.nombre}%`,
      });
    }
    if (filters.apellido) {
      qb.andWhere('cliente.apellido LIKE :apellido', {
        apellido: `%${filters.apellido}%`,
      });
    }
    if (filters.email) {
      qb.andWhere('cliente.email LIKE :email', { email: `%${filters.email}%` });
    }
    if (filters.telefono) {
      qb.andWhere('cliente.telefono LIKE :telefono', {
        telefono: `%${filters.telefono}%`,
      });
    }
    if (filters.tipo_documento_id !== undefined) {
      qb.andWhere('cliente.tipo_documento_id = :tipo_documento_id', {
        tipo_documento_id: filters.tipo_documento_id,
      });
    }
    if (filters.numero_documento) {
      qb.andWhere('cliente.numero_documento LIKE :numero_documento', {
        numero_documento: `%${filters.numero_documento}%`,
      });
    }
    if (filters.direccion) {
      qb.andWhere('cliente.direccion LIKE :direccion', {
        direccion: `%${filters.direccion}%`,
      });
    }
    if (filters.estado) {
      qb.andWhere('cliente.estado LIKE :estado', {
        estado: `%${filters.estado}%`,
      });
    }
    if (filters.agregado_en) {
      qb.andWhere('cliente.agregado_en = :agregado_en', {
        agregado_en: filters.agregado_en,
      });
    }
    if (filters.agregado_por !== undefined) {
      qb.andWhere('cliente.agregado_por = :agregado_por', {
        agregado_por: filters.agregado_por,
      });
    }
    if (filters.actualizado_en) {
      qb.andWhere('cliente.actualizado_en = :actualizado_en', {
        actualizado_en: filters.actualizado_en,
      });
    }
    if (filters.actualizado_por !== undefined) {
      qb.andWhere('cliente.actualizado_por = :actualizado_por', {
        actualizado_por: filters.actualizado_por,
      });
    }
  }

  private applySorting(qb: SelectQueryBuilder<Cliente>, sort?: string) {
    if (sort) {
      const [field, direction] = sort.split(':');
      const validFields = [
        'id',
        'empresa_id',
        'nombre',
        'apellido',
        'email',
        'telefono',
        'tipo_documento_id',
        'numero_documento',
        'direccion',
        'estado',
        'agregado_en',
        'agregado_por',
        'actualizado_en',
        'actualizado_por',
      ];
      if (
        validFields.includes(field) &&
        ['ASC', 'DESC'].includes(direction?.toUpperCase())
      ) {
        qb.orderBy(
          `cliente.${field}`,
          direction.toUpperCase() as 'ASC' | 'DESC',
        );
        return;
      }
    }
    qb.orderBy('cliente.agregado_en', 'DESC');
  }
}
